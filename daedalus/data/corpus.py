...
class BuildReport:
    ...
    #: The sampler settings this corpus was drawn with. Recorded because a
    #: corpus directory that cannot say what is in it is not a release, and
    #: "single-output" was true of every corpus ever built here without
...
def build(
    ...
    report.sampler = {
        ...
        "gate_set": list(cfg.gate_set),
        "negation_rate": cfg.negation_rate,
        "constraint_rate": cfg.constraint_rate,
    }
...
def _write_dataset_card(out: Path, report: BuildReport) -> None:
    ...
    if sampler:
        lines += [
            ...
            "| setting | value |",
            "|---|---|",
            f"| inputs | up to {sampler['max_inputs']} |",
            f"| outputs | up to {sampler['max_outputs']} |",
            f"| operators | {', '.join(sampler['gate_set'])} |",
            f"| negation rate | {sampler['negation_rate']} |",
            f"| constraint rate | {sampler['constraint_rate']} |",
        ]
    ...
    if report.sampler.get("max_outputs", 1) == 1:
        lines += [
            "- **Every circuit here has one output.** The DSL, the placer and the",
            "  verifier all handle more; the sampler draws one unless asked, because",
            "  each extra output costs roughly half the yield. A model trained on",
            "  this has never seen a circuit that drives two lamps.",
        ]
    lines += [
        "",
        "## Fields",
